""" 
    Code that uses xyz file (generated by running either Cdc42Diffusion_xyzFlowGenerator_WT or Cdc42Diffusion_xyzFlowGenerator_1ritC) that contains 3d positions and displacement vectors
    on the surface of the sphere and places the displacement vectors in bins based
    on arclength from the north pole. Displacement vectors in the same bin are averaged.
    For each bin, the average displacment vector can be converted to a flow by dividing
    by the average time between exo/endocytosis Gillespie events (avg_t).
    Output is the average flow as a function of arclength from the north pole,
    and a switch-case statement of linear functions determined by fitting line
    segments to pairs of neighboring bins.
"""
