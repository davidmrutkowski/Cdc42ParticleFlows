...
""" 
    This code calculates a heatmap of the number of the Cdc42-GTP
    on the surface from the 3d sphere simulations as in Fig 3C.
    Directory and file location of xyz's must match that which is 
    expected based on base_file_list and folder_list.
    Pulls in xyz from the third restart (Run3)
...
base_file_list = ['3d_rD_0.00020_DD_0.00030_Run3.xyz', '3d_rD_0.00045_DD_0.00030_Run3.xyz', '3d_rD_0.00100_DD_0.00030_Run3.xyz', '3d_rD_0.00224_DD_0.00030_Run3.xyz', '3d_rD_0.00500_DD_0.00030_Run3.xyz', '3d_rD_0.01118_DD_0.00030_Run3.xyz', '3d_rD_0.02500_DD_0.00030_Run3.xyz', '3d_rD_0.05590_DD_0.00030_Run3.xyz', '3d_rD_0.12500_DD_0.00030_Run3.xyz']
...
for i in range(0, len(base_file_list)):
    curr_rD = float(getValueFromString(base_file_list[i], 'rD_', '_DD'))
...
for folder in folder_list:
    ...
    for filename in base_file_list:
        ...
        curr_pos_Run3 = filename.find("_Run3")
        ...
        try:
            with open(filepath) as fp:
                    ...
                    if len(split_string) == 1:
                        ...
                        for n in range(0, num_particles):
                            ...
                            if len(split_string) >= 4:
                                ...
                                particles[temp_type] += 1
                                
                        if(frame_count > frame_cutoff):
                          particle_avg_list.append(particles[:])  
                                
                            
                    line = fp.readline()
                
                avg_num_GTP = np.sum([elem[1] for elem in particle_avg_list]) / len(particle_avg_list)
                avg_num_GDP_membrane = np.sum([elem[0] for elem in particle_avg_list]) / len(particle_avg_list)
                avg_num_Scd2GTP_membrane = np.sum([elem[5] for elem in particle_avg_list]) / len(particle_avg_list)
                
                first_pos = filepath.find("rD_")
                
                tmp_str = filepath[first_pos+3:]
                ...
                print(curr_rD, curr_DD_folder, avg_num_GTP + avg_num_Scd2GTP_membrane)
                
                C[curr_pos_rD, curr_pos_DD] = avg_num_GTP + avg_num_Scd2GTP_membrane
...
im = plt.pcolormesh(DD_mesh, rD_mesh, C, cmap=plt.cm.Greens, vmin=150.0, vmax=800.0)
cbar = plt.colorbar(im)
cbar.ax.tick_params(labelsize=16) 
cbar.ax.set_title('# Cdc42-GTP', size=18, x=0.75, y=1.01)
